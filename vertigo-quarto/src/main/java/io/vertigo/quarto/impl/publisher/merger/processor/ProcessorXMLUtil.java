...
public final class ProcessorXMLUtil {
	 ...
	 */
	public static Deque<TagXML> extractUnbalancedTag(final char[] content) {
	 ...
	 * @param xmlContent Input XML content
	 * @param tag Trigger tag
	 * @param handler XML handler of this tag
	...
	public static StringBuilder parseXMLContent(final String xmlContent, final String tag, final ParserXMLHandler handler) {
		...
		while (index < xmlContent.length()) {
			//TODO Assertion.invariant(indexNoBodyMatch == indexBodyMatch && indexBodyMatch != -1, "Cas non prévu : {0} == {1}", bodyMatcher.group(), noBodyMatcher.group());
