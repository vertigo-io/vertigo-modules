...
public class EasyFormsRunnerServices implements IEasyFormsRunnerServices {
	...
	@Override
	public void formatAndCheckFormulaire(final Entity formOwner, final EasyFormsData formData, final EasyFormsTemplate formTempalte, final UiMessageStack uiMessageStack) {
		Assertion.check()
		...
		for (final var section : formTempalte.getSections()) {
			...
			final EasyFormsData formattedFormDataSection = new EasyFormsData();
			...
			for (final var elem : section.getItems()) {
				if (elem instanceof final EasyFormsTemplateItemBlock block) {
					// test block condition, then process items or else continue;
					continue;
				} else if (elem instanceof final EasyFormsTemplateItemField field) {
					final var formattedValue = formatAndCheckField(field, formData, formOwner, uiMessageStack);
	...
	private Object formatAndCheckField(final EasyFormsTemplateItemField field, final EasyFormsData formData, final Entity formOwner, final UiMessageStack uiMessageStack) {
